...
Section Environment.
  ...
  Definition env := seq (K * V).
  (* Definition nil : env := []. *)
  (* Definition undef_env : env := Undef. *) (* ??? *)

  (* equality of environments *) (* seq is already an eqtype *)
  ...
  Fixpoint look x (E : env) : option V :=
    match E with
    | nil => None
  ...
  Definition dom (D : env) := if def D then map fst D else [::].
  ...
  Proof.
    elim D=>// f.
    rewrite/add/add_upd/map in_nil /look eq_refl=>//.
  ...
  Definition filter_by_key (pr : pred K) (E: env) : env := filter (fun en => pr (fst en)) E.
  ...
  Fixpoint map_on_keys' (f : K -> K) (s : seq (K * V)) : seq (K * V) :=
