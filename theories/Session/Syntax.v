    ...
    => (a1 == a2)
         && (fix eqK Ks1 Ks2 :=
               ...
               | K1::Ks1, K2::Ks2 =>
                 (K1.1 == K2.1)
                   && (K1.2.1 == K2.2.1)
                   && eq_sty K1.2.2 K2.2.2 && eqK Ks1 Ks2
               | _, _ => false
               end) Ks1 Ks2
  | _, _ => false
  end.
...
Definition eq_salt (l r : lbl * (mty * s_ty)) :=
  (l.1 == r.1) && (l.2.1 == r.2.1) && eq_sty l.2.2 r.2.2.
...
Fixpoint s_open (d : nat) (S2 : s_ty) (S1 : s_ty) :=
  ...
  | s_msg a K => s_msg a [seq (lS.1, (lS.2.1, s_open d S2 lS.2.2)) | lS <- K]
  end.
...
Lemma sty_ind :
  forall (P : s_ty -> Prop),
    ...
    (forall a K, Forall (fun lS => P lS.2.2) K -> P (s_msg a K)) ->
    forall s : s_ty, P s.
Proof.
...
Fixpoint depth_sty L :=
  ...
  | s_msg _ K  => (foldr maxn 0 [seq depth_sty l.2.2 | l <- K]).+1
  end.
...
Fixpoint dual (L : s_ty) : s_ty :=
  ...
  | s_msg a K => s_msg (dual_act a) [seq (x.1, (x.2.1, dual x.2.2)) | x <- K]
  end.
